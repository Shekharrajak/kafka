...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handleTxnShareAcknowledgeRequest(request: Request, requestLocal: RequestLocal): CompletableFuture[Unit] = {
    ...
    val shareStatePartitions: util.Set[TopicPartition] = new util.HashSet[TopicPartition]()
    acknowledgeBatchesMap.keys.foreach { tip =>
      val partitionIndex = shareCoordinator.partitionFor(SharePartitionKey.getInstance(groupId, tip))
      shareStatePartitions.add(new TopicPartition(SHARE_GROUP_STATE_TOPIC_NAME, partitionIndex))
    }

    val registrationDone = new CompletableFuture[Unit]()
    txnCoordinator.handleAddPartitionsToTransaction(
      transactionalId, producerId, producerEpoch, shareStatePartitions,
      (error: Errors) => {
        if (error eq Errors.NONE) registrationDone.complete(())
        else registrationDone.completeExceptionally(error.exception)
      },
      TransactionVersion.TV_2, requestLocal)

    registrationDone.thenCompose[Unit] { _ =>
      ...
      requestHelper.sendMaybeThrottle(request, txnShareAcknowledgeRequest.getErrorResponse(AbstractResponse.DEFAULT_THROTTLE_TIME, ex))
