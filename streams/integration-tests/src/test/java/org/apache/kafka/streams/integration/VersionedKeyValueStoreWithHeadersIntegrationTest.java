...
public class VersionedKeyValueStoreWithHeadersIntegrationTest {
    ...
    public void shouldPutAndGetWithHeaders() throws Exception {
        ...
        final List<KeyValue<Integer, Integer>> results =
            IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(
                consumerConfig(), outputStream, 1, Duration.ofSeconds(30).toMillis());
    ...
    public void shouldPutAndGetWithEmptyHeaders() throws Exception {
        ...
        produceDataToTopicWithHeaders(inputStream, baseTimestamp, EMPTY_HEADERS,
            new KeyValue<>(1, "value1"));
    ...
    public void shouldPreserveHeadersAcrossMultipleVersions() throws Exception {
        ...
        produceDataToTopicWithHeaders(inputStream, baseTimestamp + 1000, HEADERS2,
            new KeyValue<>(1, "version2"));

        final List<KeyValue<Integer, Integer>> results2 =
            IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(
                consumerConfig(), outputStream, 2, Duration.ofSeconds(30).toMillis());
    ...
    public void shouldRestoreHeadersFromChangelog() throws Exception {
        ...
        processor2.seedExpectedData(1, "restored-value", baseTimestamp, HEADERS1);
        ...
        produceDataToTopicWithHeaders(inputStream, baseTimestamp + 5000, EMPTY_HEADERS,
            new KeyValue<>(99, "trigger"));

        final List<KeyValue<Integer, Integer>> secondRunResults =
            IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(
                consumerConfig(), outputStream, 1, Duration.ofSeconds(30).toMillis());
        assertEquals(0, secondRunResults.get(0).value, "Restoration check failed: headers not preserved");
    }
    ...
    private Properties consumerConfig() {
        ...
        config.put(ConsumerConfig.GROUP_ID_CONFIG, "versioned-headers-it-consumer");
